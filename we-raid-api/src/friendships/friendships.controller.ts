...
export class FriendshipsController {
  ...
  @Get()
  @ApiOperation({ summary: '친구 목록 조회 (status 필터: PENDING | ACCEPTED | BLOCKED)' })
  getFriendships(@CurrentUser() user: AuthUser, @Query() query: QueryFriendshipDto) {
  ...
  acceptRequest(@CurrentUser() user: AuthUser, @Param('requesterId') requesterId: string) {
    return this.friendshipsService.acceptRequest(user.id, requesterId)
  }

  @Patch(':userId/block')
  @ApiOperation({ summary: '유저 차단' })
  blockUser(@CurrentUser() user: AuthUser, @Param('userId') targetUserId: string) {
  ...
  removeFriendship(@CurrentUser() user: AuthUser, @Param('userId') targetUserId: string) {
