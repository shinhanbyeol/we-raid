...
const SCHEDULE_BASE_SELECT = {
  ...
  _count: { select: { participants: { where: { status: 'ACCEPTED' as const } } } },
}
...
export class SchedulesService {
  ...
  async getMySchedules(userId: string, query: QueryScheduleDto) {
    const where: Record<string, any> = {
      participants: { some: { userId, status: { in: ['ACCEPTED', 'PENDING', 'WAITLIST'] } } },
    }
  ...
  async createSchedule(userId: string, dto: CreateScheduleDto) {
    ...
    if (startAt >= endAt) {
      throw new AppException('WR-SCHED-001', '종료 시간이 시작 시간보다 늦어야 합니다', HttpStatus.BAD_REQUEST)
    }

    if (dto.recurringRule) this.validateRRule(dto.recurringRule)

    const game = await this.prisma.game.findUnique({ where: { id: dto.gameId, isActive: true } })
    if (!game) throw new AppException('WR-SCHED-002', '게임을 찾을 수 없습니다', HttpStatus.NOT_FOUND)
    ...
    if (dto.groupId) {
      ...
      if (!member || member.status !== 'ACTIVE') {
        throw new AppException('WR-SCHED-003', '그룹 멤버만 그룹 일정을 생성할 수 있습니다', HttpStatus.FORBIDDEN)
      }
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.participant.create({
        data: { scheduleId: schedule.id, userId, status: 'ACCEPTED', respondedAt: new Date() },
      })
  ...
  async getSchedule(userId: string, scheduleId: string) {
    ...
    if (!schedule) throw new AppException('WR-SCHED-004', '일정을 찾을 수 없습니다', HttpStatus.NOT_FOUND)
    ...
    if (schedule.group) {
      ...
      if (!member || member.status !== 'ACTIVE') {
        throw new AppException('WR-SCHED-005', '그룹 멤버만 조회할 수 있습니다', HttpStatus.FORBIDDEN)
      }
    }

    // recurringRule이 있으면 다음 10회 발생 일자 파싱하여 포함
    const recurringOccurrences = this.parseNextOccurrences(schedule.recurringRule, schedule.startAt)
  ...
  async cancelSchedule(userId: string, scheduleId: string) {
    ...
    const participants = await this.prisma.participant.findMany({
      where: { scheduleId, status: { in: ['ACCEPTED', 'PENDING'] }, userId: { not: userId } },
      select: { userId: true },
    })
    await this.notifications.notifyMany(
      participants.map((p) => p.userId),
  ...
  async inviteParticipant(userId: string, scheduleId: string, dto: InviteParticipantDto) {
    const schedule = await this.assertLeader(userId, scheduleId)
    this.assertNotCancelled(schedule)
    if (schedule.status === 'CLOSED') {
      throw new AppException('WR-SCHED-006', '마감된 일정에는 초대할 수 없습니다', HttpStatus.BAD_REQUEST)
    }

    const targetUser = await this.prisma.user.findUnique({ where: { id: dto.userId } })
    if (!targetUser || targetUser.status !== 'ACTIVE') {
      throw new AppException('WR-SCHED-007', '유저를 찾을 수 없습니다', HttpStatus.NOT_FOUND)
    }

    const existing = await this.prisma.participant.findUnique({
      where: { scheduleId_userId: { scheduleId, userId: dto.userId } },
    })
    if (existing) throw new AppException('WR-SCHED-008', '이미 초대된 유저입니다', HttpStatus.CONFLICT)
  ...
  async respondParticipant(userId: string, scheduleId: string, dto: RespondParticipantDto) {
    ...
    if (!participant) throw new AppException('WR-SCHED-009', '초대 내역이 없습니다', HttpStatus.NOT_FOUND)

    if (participant.status === 'ACCEPTED') {
      throw new AppException('WR-SCHED-010', '이미 수락한 초대입니다', HttpStatus.BAD_REQUEST)
    }
    if (participant.status === 'DECLINED') {
      throw new AppException('WR-SCHED-011', '이미 거절한 초대입니다', HttpStatus.BAD_REQUEST)
    }
    if (participant.status === 'WAITLIST') {
      throw new AppException('WR-SCHED-015', '대기열에 있습니다. 자리가 생기면 알림을 받게 됩니다', HttpStatus.BAD_REQUEST)
    }

    // 수락 처리
    if (dto.status === 'ACCEPTED') {
      // 시간 충돌 감지
      await this.detectConflict(userId, scheduleId, schedule.startAt, schedule.endAt)
  ...
  async getPublicSchedules(query: QueryPublicScheduleDto) {
    ...
    return this.prisma.schedule.findMany({
      ...
      select: {
        ...SCHEDULE_BASE_SELECT,
        slots: { select: { id: true, role: true, count: true, filledCount: true } },
      },
      orderBy: { startAt: 'asc' },
    })
  }
  ...
  async setSlots(userId: string, scheduleId: string, dto: CreateSlotsDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.scheduleSlot.createMany({
        data: dto.slots.map((s) => ({ scheduleId, role: s.role, count: s.count })),
      })
  ...
  private async assertLeader(userId: string, scheduleId: string) {
    ...
    if (schedule.leaderId !== userId) {
      throw new AppException('WR-SCHED-012', '리더만 수정할 수 있습니다', HttpStatus.FORBIDDEN)
    }
    return schedule
  }
  ...
  private assertNotCancelled(schedule: { status: string }) {
    if (schedule.status === 'CANCELLED') {
      throw new AppException('WR-SCHED-013', '취소된 일정입니다', HttpStatus.BAD_REQUEST)
    }
  }

  /** 동일 userId의 ACCEPTED 일정과 시간 충돌 감지 */
  private async detectConflict(userId: string, excludeScheduleId: string, startAt: Date, endAt: Date) {
    const conflict = await this.prisma.participant.findFirst({
      ...
      include: { schedule: { select: { id: true, title: true, startAt: true, endAt: true } } },
    })
  ...
  private validateRRule(rule: string) {
    try {
      ...
      throw new AppException('WR-SCHED-016', '유효하지 않은 RRULE 형식입니다', HttpStatus.BAD_REQUEST)
    }
  }

  /** recurringRule로부터 dtstart 기준 다음 N회 발생 일자 반환 */
  private parseNextOccurrences(rule: string | null, dtstart: Date, count = 10): Date[] {
    if (!rule) return []
    try {
