...
export class AdminService {
  ...
  async createGame(dto: CreateGameDto) {
    const exists = await this.prisma.game.findUnique({ where: { slug: dto.slug } })
    if (exists) throw new ConflictException('WR-ADMIN-010', '이미 존재하는 슬러그입니다')
    return this.prisma.game.create({ data: { ...dto, config: dto.config ?? '{}' } })
  }

  async updateGame(id: string, dto: UpdateGameDto) {
    await this.assertGame(id)
    if (dto.slug) {
      const exists = await this.prisma.game.findFirst({ where: { slug: dto.slug, NOT: { id } } })
      if (exists) throw new ConflictException('WR-ADMIN-010', '이미 존재하는 슬러그입니다')
    }
  ...
  async createServer(gameId: string, dto: CreateServerDto) {
    ...
    return this.prisma.gameServer.create({ data: { gameId, ...dto, displayOrder: dto.displayOrder ?? 0 } })
  }
  ...
  getUsers() {
    return this.prisma.user.findMany({
      select: { id: true, kakaoId: true, nickname: true, profileImage: true, status: true, isAdmin: true, createdAt: true },
  ...
  async updateUserStatus(id: string, dto: UpdateUserStatusDto) {
    ...
    if (!user) throw new NotFoundException('WR-USER-003', '사용자를 찾을 수 없습니다')
  ...
  private async assertGame(id: string) {
    ...
    if (!game) throw new NotFoundException('WR-GAME-001', '게임을 찾을 수 없습니다')
  }

  private async assertServer(gameId: string, id: string) {
    const server = await this.prisma.gameServer.findFirst({ where: { id, gameId } })
