...
export class CharactersService {
  ...
  async create(userId: string, dto: CreateCharacterDto) {
    ...
    if (dto.mainCharId) {
      if (isMain) throw new BadRequestException('WR-CHAR-001', '본캐에는 mainCharId를 지정할 수 없습니다')
  ...
  async update(userId: string, id: string, dto: UpdateCharacterDto) {
    ...
    if (dto.isMain === true && char.mainCharId) {
      ...
      dto = { ...dto, mainCharId: null }
    }

    if (dto.mainCharId && dto.isMain !== false && char.isMain) {
      throw new BadRequestException('WR-CHAR-001', '본캐에는 mainCharId를 지정할 수 없습니다')
    }

    if (dto.mainCharId) {
      await this.validateMainChar(userId, char.gameId, dto.mainCharId)
    }

    if (dto.serverId) {
      await this.validateServer(char.gameId, dto.serverId)
    }
  ...
  private async assertOwner(userId: string, id: string) {
    ...
    if (char.userId !== userId) throw new ForbiddenException('WR-CHAR-003', '본인 캐릭터만 수정할 수 있습니다')
    return char
  }

  private async validateMainChar(userId: string, gameId: string, mainCharId: string) {
    const main = await this.prisma.character.findUnique({ where: { id: mainCharId } })
    if (!main || main.isDeleted) throw new NotFoundException('WR-CHAR-002', '본캐를 찾을 수 없습니다')
    if (main.userId !== userId) throw new ForbiddenException('WR-CHAR-004', '본인 캐릭터만 본캐로 연결할 수 있습니다')
    if (main.gameId !== gameId) throw new BadRequestException('WR-CHAR-005', '본캐와 동일한 게임이어야 합니다')
    if (!main.isMain) throw new BadRequestException('WR-CHAR-006', '부캐를 본캐로 연결할 수 없습니다')
  }

  private async validateServer(gameId: string, serverId: string) {
    const server = await this.prisma.gameServer.findFirst({ where: { id: serverId, gameId, isActive: true } })
    if (!server) throw new NotFoundException('WR-GAME-002', '서버를 찾을 수 없습니다')
  }
}
